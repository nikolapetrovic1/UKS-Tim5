...
def milestone_progress(milestone: Milestone):
      closed_issues = Issue.objects.filter(milestone = milestone, state= IssueState.CLOSED).count()
      open_issues = Issue.objects.filter(milestone = milestone, state= IssueState.OPEN).count()
